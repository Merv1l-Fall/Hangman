...
function showNextPart() {
	if (incorrectGuesses < hangmanParts.length) {
		console.log(
			`Visar del ${incorrectGuesses}: ${hangmanParts[incorrectGuesses].id}`
		);
...
function handleIncorrectGuess() {
	...
	if (incorrectGuesses >= maxIncorrectGuesses) {
		...
		gameOver.style.display = "flex"; // Show overlay
	}

	document.querySelector("#restartGame").addEventListener("click", (e) => {
		e.preventDefault(); 
		document.querySelector("#gameOver").style.display = "none"; // Hide overlay
		startNewGame(); // Restart game logic
	});
}
...
function checkWin() {
	const blanks = document.querySelectorAll(".gissa");
	const win = Array.from(blanks).every(
		(blank, index) => blank.textContent === secretWord[index]
	);
	if (win) {
